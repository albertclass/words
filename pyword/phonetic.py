...
class youdao():
    def __init__(self, type: int = 0, rootpath: str = './'):
        '''
        调用youdao API
        type = 0：美音
        type = 1：英音
        ...
        # 文件根目录
        self._dirRoot = os.path.dirname(os.path.abspath(rootpath))
        if 0 == self._type:
            self._dirSpeech = os.path.join(self._dirRoot, 'us')  # 美音库
        else:
            self._dirSpeech = os.path.join(self._dirRoot, 'en')  # 英音库
    ...
    def down(self, word):
        ...
        if tmp is None:
            urlpath = 'http://dict.youdao.com/dictvoice?type=' + str(self._type) + '&audio=' + self._word
            # 调用下载程序，下载到目标文件夹
            # print('不存在 %s.mp3 文件\n将URL:\n' % word, self._url, '\n下载到:\n', self._filePath)
            # 下载到目标地址
            urllib.request.urlretrieve(urlpath, filename=tmp)
            print('%s.mp3 下载完成' % word)
        else:
